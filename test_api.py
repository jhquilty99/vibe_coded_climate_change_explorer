...
async def test_temperature_summary(api_client, coord, expected_earliest_temp):
    """Test temperature summary endpoint with different coordinates and validate earliest temperature values"""
    async with api_client as client:
        url = f"{BASE_URL}/api/v1/temperature-summary"
        params = {"lat": coord["lat"], "lng": coord["lng"]}
        ...
        assert "earliest_temperature" in data
        ...
        assert isinstance(data["earliest_temperature"], (int, float))
        assert isinstance(data["temperature_difference"], (int, float))
        
        # Validate earliest temperature values with margin of error
        actual_temp = data["earliest_temperature"]
        margin_of_error = 0.1
...
async def test_temperature_graph(api_client, coord):
    """Test temperature graph endpoint with different coordinates"""
    async with api_client as client:
        url = f"{BASE_URL}/api/v1/temperature-graph"
        params = {"lat": coord["lat"], "lng": coord["lng"]}
...
async def test_missing_parameters(api_client):
    ...
    async with api_client as client:
        ...
        # Test temperature summary without parameters
        url = f"{BASE_URL}/api/v1/temperature-summary"
        response = await client.get(url)
        assert response.status_code == 422  # Unprocessable Entity
        
        # Test temperature graph without parameters
        url = f"{BASE_URL}/api/v1/temperature-graph"
        response = await client.get(url)
        assert response.status_code == 422  # Unprocessable Entity
...
async def test_1940_temperature_consistency(api_client, coord):
    """Test that 1940 average temperature from graph endpoint matches earliest temperature from summary endpoint"""
    async with api_client as client:
        # Get temperature summary to get earliest temperature
        summary_url = f"{BASE_URL}/api/v1/temperature-summary"
        summary_params = {"lat": coord["lat"], "lng": coord["lng"]}
        
        summary_response = await client.get(summary_url, params=summary_params)
        assert summary_response.status_code == 200
        summary_data = summary_response.json()
        
        earliest_temp = summary_data["earliest_temperature"]
        
        # Get temperature graph data
        graph_url = f"{BASE_URL}/api/v1/temperature-graph"
        graph_response = await client.get(graph_url, params=summary_params)
        assert graph_response.status_code == 200
        graph_data = graph_response.json()
        
        # Find 1940 data in the graph
        temp_1940 = None
        for year_data in graph_data:
...
async def test_api_response_times(api_client):
    ...
    async with api_client as client:
        ...
        # Test temperature summary response time
        start_time = time.time()
        url = f"{BASE_URL}/api/v1/temperature-summary"
        response = await client.get(url, params=params)
        end_time = time.time()
